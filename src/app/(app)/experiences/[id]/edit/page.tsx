...
export default function EditExperiencePage() {
  ...
  const [form, setForm] = useState({
    title: "", destination: "", tripDate: "", coverImage: "",
    excerpt: "", content: "", rating: 0, mood: "", tags: "",
  });
  ...
  useEffect(() => {
    fetch(`/api/experiences/${id}`)
      ...
      .then((data) => {
        ...
        if (data.destination) setDestValidated(true);
        setLoading(false);
      });
  }, [id]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!destValidated) {
      setError("Selecione um destino real a partir das sugestões no campo Destino.");
      return;
    }
    setSaving(true);
    setError("");
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 pb-12">
      <div className="flex items-center gap-4">
        <Link href={`/experiences/${id}`} className="p-2 rounded-lg border border-gray-200 text-gray-500 hover:text-gray-800 hover:bg-gray-50 transition-colors">
          <ArrowLeft className="h-4 w-4" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Editar Relato</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <LocationInput
                ...
                onChange={(val, validated) => { set("destination", val); setDestValidated(validated); }}
            ...
            <div className="space-y-1.5">
              <Label>Período *</Label>
              <Input value={form.tripDate} onChange={(e) => set("tripDate", e.target.value)} required />
            </div>
          </div>
          <div className="space-y-1.5">
            <Label>Resumo</Label>
        ...
        <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
            ...
            <div className="flex gap-1 text-xs">
              {(["preset", "url"] as const).map((m) => (
                <button key={m} type="button" onClick={() => setCoverMode(m)}
                  className={cn("px-3 py-1 rounded-lg font-medium transition-colors", coverMode === m ? "bg-primary-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200")}>
          ...
          {coverMode === "preset" ? (
            <div className="grid grid-cols-4 gap-2">
              {COVER_PRESETS.map((p) => (
                <button key={p.url} type="button" onClick={() => set("coverImage", form.coverImage === p.url ? "" : p.url)}
                  className={cn("relative rounded-xl overflow-hidden h-16 border-2 transition-all", form.coverImage === p.url ? "border-primary-500 ring-2 ring-primary-400/30" : "border-transparent hover:border-gray-300")}>
          ...
          {form.coverImage && (
            <div className="relative rounded-xl overflow-hidden h-40">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={form.coverImage} alt="Capa" className="w-full h-full object-cover" onError={() => set("coverImage", "")} />
              <button type="button" onClick={() => set("coverImage", "")} className="absolute top-2 right-2 w-7 h-7 rounded-lg bg-black/50 text-white flex items-center justify-center hover:bg-black/70 transition-colors">
        ...
        <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {MOODS.map((m) => (
                <button key={m.value} type="button" onClick={() => set("mood", form.mood === m.value ? "" : m.value)}
                  className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-sm font-medium border transition-all", form.mood === m.value ? "bg-primary-600 text-white border-primary-600 shadow-sm" : "bg-white text-gray-600 border-gray-200 hover:border-gray-300")}>
          ...
          <div className="space-y-1.5">
            <Label>Tags</Label>
            <Input value={form.tags} onChange={(e) => set("tags", e.target.value)} placeholder="Ex: gastronomia, cultura, família" />
          </div>
        </section>
